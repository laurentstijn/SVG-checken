...
svg.addEventListener("mousedown", (e) => {
  if (mode === "move" || e.target.closest('svg') !== svg) return;
  isDrawing = true;
  startX = e.offsetX;
  startY = e.offsetY;

  previewShape = document.createElementNS("http://www.w3.org/2000/svg", mode === "circle" ? "circle" : "rect");
  previewShape.setAttribute("fill", "#00aaff");
  previewShape.setAttribute("stroke", "#333");
...
svg.addEventListener("mousemove", (e) => {
  ...
  if (mode === "circle") {
    previewShape.setAttribute("cx", x + w / 2);
    previewShape.setAttribute("cy", y + h / 2);
    previewShape.setAttribute("r", Math.sqrt(w * w + h * h) / 2);
  } else {
    previewShape.setAttribute("x", Math.min(x, e.offsetX));
    previewShape.setAttribute("y", Math.min(y, e.offsetY));
    previewShape.setAttribute("width", Math.abs(w));
    previewShape.setAttribute("height", Math.abs(h));
  }
});
...
svg.addEventListener("mouseup", () => {
  ...
  previewShape.addEventListener("click", () => selectedElement = previewShape);
...
window.maakNieuweSVG = () => {
  while (svg.firstChild) svg.removeChild(svg.firstChild); // Leeg canvas echt volledig
  laatstGebruikteBestandsnaam = '';
  svgDropdown.value = '';

  // Voeg optioneel een lege achtergrond toe (optioneel)
